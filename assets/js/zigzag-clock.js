...
// the inverted geometry leaves empty. The dial is numbered from zero, 0 at the
// top, so it does not borrow the 12-at-the-top of our own clocks.
//
// The digital readout below the dial gives the whole 20-hour count, 00 to 19,
// rather than the dial's half-day reading: the two are meant to be read
// differently, and only the dial needs the half-day indicator to be unambiguous.
//
...
(function () {
  ...
  // The two halves of the day, for the dial's aperture alone: the readout gives
  // the whole 20-hour count and so needs no half indicator, in writing or in
  // speech. Rename this if the fiction ever settles on names for them — the
  // aperture wants a single glyph.
  var HALF_GLYPH = ['i', 'ii'];
  ...
  // Split a day count into its fields. hour20 is the 20-hour reading (0–19) the
  // digital readout carries; hour10 is the half-day reading on the dial (0–9),
  // which counts from zero as the face is numbered — there is no 12-at-the-top
  // convention here to borrow.
  ...
  function format(count) {
    ...
    return {
      ...
      spoken: t.hour20 + ' hours, ' + t.minute + ' minutes, ' + t.second +
              ' seconds'
    };
  }
  ...
  function dialMarkup() {
    ...
    // Numerals 1–9 outside the hour notches, and 0 at the top: the dial counts
    // from zero rather than borrowing the 12-at-the-top of our own clocks.
    var numerals = '';
    for (i = 1; i <= 10; i++) {
      var q = polar(i * 36, F.numeral * R);
      numerals += '<text class="zz-numeral" x="' + n2(q[0]) + '" y="' + n2(q[1]) +
        '" text-anchor="middle" dominant-baseline="central">' + (i % 10) + '</text>';
    }
  ...
  function readoutMarkup() {
    return '' +
      '<div class="zz-readout" role="timer" aria-live="off" aria-label="Loading the time">' +
        '<div class="zz-time">' +
          '<span class="zz-h">00</span><span class="zz-sep">:</span>' +
  ...
  function init(container) {
    ...
    var elH = q('.zz-h'), elM = q('.zz-m'), elS = q('.zz-s');
    ...
    function frame(ts) {
      ...
      if (N !== lastSecond) {
        ...
        if (t.hour20 * MIN_PER_HOUR + t.minute !== lastMinute) {
          ...
          elH.textContent = pad(t.hour20, 2);
